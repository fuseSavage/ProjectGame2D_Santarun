...
func _process(delta):
	var LabelNode = get_parent().get_parent().get_node("Mainscn2/ScoreCounty/UI/Bese/RichTextLabel2")
		...
		if get_slide_count() > 0:
			for i in range(get_slide_count()):
				...
				if "door" in get_slide_collision(i).collider.name:
					get_tree().change_scene("Youwin.tscn")
...
func _on_Timer_timeout():
	...
	if (Health == 3):
		var spritehealth_1 = get_parent().get_parent().get_node("Mainscn2/NodeHealth/aa/3")
		spritehealth_1.hide()
		$AnimatedSprite.play("dying")
	if (Health == 2):
		var spritehealth_2 = get_parent().get_parent().get_node("Mainscn2/NodeHealth/aa/2")
		spritehealth_2.hide()
		$AnimatedSprite.play("dying")
